...
open class SwipeMenuView: UIView {
    ...
    open weak var contentMiddleView: UIView?
    ...
    open fileprivate(set) var middleView: UIView? {
        didSet {
            guard let middleView = middleView else { return }
            addSubview(middleView)
            bringSubviewToFront(middleView)
            middleView.frame = CGRect(x: 0, y: options.tabView.height + 10, width: frame.width, height: options.tabView.height)
            middleView.backgroundColor = options.tabView.backgroundColor
            middleView.clipsToBounds = options.tabView.clipsToBounds
            layout(middleView: middleView)
        }
    }
    ...
    private func setup(default defaultIndex: Int = 0) {
        ...
        backgroundColor = options.tabView.backgroundColor// .clear
        ...
        if let contentMiddleView = contentMiddleView {
            middleView = contentMiddleView
            //middleView?.frame = CGRect(x: 0, y: options.tabView.height + 10, width: frame.width, height: 20)
            //middleView = UIView(frame: )
            //middleView?.backgroundColor = options.tabView.backgroundColor
            //middleView?.clipsToBounds = false
            //middleView?.addSubview(contentMiddleView)
            //middleView?.clipsToBounds = options.tabView.clipsToBounds
        }

        contentScrollView = ContentScrollView(frame: CGRect(x: 0, y: options.tabView.height * 2 + 20, width: frame.width, height: frame.height - (options.tabView.height * 2 + 20)), default: defaultIndex, options: options.contentScrollView)
    ...
    private func layout(middleView: UIView) {
        middleView.translatesAutoresizingMaskIntoConstraints = false
        NSLayoutConstraint.activate([
            middleView.topAnchor.constraint(equalTo: self.topAnchor, constant: options.tabView.height + 10.0),
            ...
            middleView.heightAnchor.constraint(equalToConstant: options.tabView.height)
        ])
    ...
    private func layout(contentScrollView: ContentScrollView) {
        ...
        NSLayoutConstraint.activate([
            contentScrollView.topAnchor.constraint(equalTo: self.topAnchor, constant: options.tabView.height*2 + 20),
